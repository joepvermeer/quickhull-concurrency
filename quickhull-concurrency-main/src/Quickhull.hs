...
initialPartition points =
  let
      ...
      newPoints = 
        let
          Z :. n = unlift (shape points) :: Z :. Exp Int
          total  = 3 + the countUpper + the countLower
        in
        permute
          (\_new -> new)
          (generate (index1 total) (\ix -> p1))
          destination
          points
          ...
          `awhile`
            ...
            (\arr -> arr
              & fillAt (index1 0) p1
              & fillAt (index1 (1 + the countUpper)) p2
              & fillAt (index1 (2 + the countUpper + the countLower)) p1)
      ...
      headFlags = 
        ...
        generate (index1 total) $ \ix ->
          ...
          in i==0 || i==1 + the countUpper || i==2 + the countUpper + the countLower
...
partition (T2 headFlags points) =
  let
    leftPts = propogateL headFlags points
    rightPts = propogateR headFlags points

    distances = 
      zipWith
        (\p l ->
          nonNormalizedDistance(T2 l (rightPts ! index1 0)) p)
        points
        leftPts
    ...
    rightOfRight =
      zipWith3
        (\p l r -> pointIsLeftOfLine (T2 p l) p)
        points
        leftPts
        rightPts
    ...
    flags' = pack keep newHeadFlags
    points' = pack keep points
    in
  T2 flags' points'
...
quickhull pts =
  let
    initial = initialPartition leftPts
      ...
      T2 finalFlags finalPoints = awhile done step initial
  in
  pack finalFlags finalPoints
